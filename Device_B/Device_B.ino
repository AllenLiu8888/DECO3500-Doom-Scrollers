/* =============================================================================
 *  Dual ESP-NOW Pomodoro System (Auto-Resume) - Device B
 *
...
// #define IS_A_SIDE 1  // Not defined on Device B

// ────────────────────────────────────────────────────────────────────────────
// 2. Top-level configuration
 // ────────────────────────────────────────────────────────────────────────────
const char* MY_NAME = "B";  // For logging: identify local device role

// Peer (A) STA MAC — paste A's MAC printed by TestingBeforeStart here
...
float NO_PHONE_VAL    = -39732;
float WITH_PHONE_VAL  =   8959;
float DETECT_MARGIN   =  1200;
...
bool ready_peer   = false;
bool readyAckSent = false;
...
void onRecv(const esp_now_recv_info* info, const uint8_t* data, int len){
  ...
  switch(m.type){
    case MSG_READY: {
      ...
      // Device B: once both READY, wait for A's START
...
void loop(){
  ...
  switch(state){
    ...
    case FOCUS:
      ...
      if (now - stateStartMs >= FOCUS_MS){
        state = BREAK_TIME; stateStartMs = now;
        lcdWrite16(0,"Break time     "); lcdWrite16(1,"Relax   05:00  ");
        break;
      }
      if (now - lastUiMs >= 250){
        lastUiMs = now;
